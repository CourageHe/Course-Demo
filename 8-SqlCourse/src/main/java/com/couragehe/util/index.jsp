...
{ 
var mm="NewFile.jsp?month="+document.sm.elements[0].selectedIndex+"&year=" 
+<%=year%>
	;
...
%>
<body>
	<FORM name="sm" method="post" action="NewFile.jsp">
		<%=year%>年
...
nter"><a
						href="jump.jsp?year=<%=year%>&month=<%=Integer.parseInt(month) + 1%>&d 
ate=<%=days[i]%>"
		...
		<table border="0" width="168" height="20">
			<tr>
				...
				<td width=28%><input type=text name="year" value=<%=year%>
